...
(async function() {
  ...
  console.log(`Deploying sToken with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('SErc20Immutable', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'SErc20Immutable', deployArgs, 0);
