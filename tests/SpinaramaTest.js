...
describe('Spinarama', () => {
  ...
  describe('#mintMint', () => {
    it('should succeed', async () => {
      ...
      await send(sToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(sToken.underlying, 'approve', [sToken._address, -1], {from});
      await minerStop();
      const p1 = send(sToken, 'mint', [1], {from});
      const p2 = send(sToken, 'mint', [2], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(sToken, from)).toEqualNumber(3);
    });

    it('should partial succeed', async () => {
      const sToken = await makeSToken({supportMarket: true});
      await send(sToken.underlying, 'harnessSetBalance', [from, 100], {from});
      await send(sToken.underlying, 'approve', [sToken._address, 10], {from});
      await minerStop();
      const p1 = send(sToken, 'mint', [11], {from});
      const p2 = send(sToken, 'mint', [10], {from});
      ...
      expect(await balanceOf(sToken, from)).toEqualNumber(10);
    });
  });
  ...
  describe('#mintRedeem', () => {
    it('should succeed', async () => {
      ...
      await send(sToken.underlying, 'approve', [sToken._address, 10], {from});
      await minerStop();
      const p1 = send(sToken, 'mint', [10], {from});
      const p2 = send(sToken, 'redeemUnderlying', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect(await balanceOf(sToken, from)).toEqualNumber(0);
    });
  });
  ...
  describe('#redeemMint', () => {
    it('should succeed', async () => {
      const sToken = await makeSToken({supportMarket: true});
      await send(sToken, 'harnessSetTotalSupply', [10]);
      await send(sToken, 'harnessSetExchangeRate', [etherMantissa(1)]);
      await send(sToken, 'harnessSetBalance', [from, 10]);
      await send(sToken.underlying, 'harnessSetBalance', [sToken._address, 10]);
      await send(sToken.underlying, 'approve', [sToken._address, 10], {from});
      await minerStop();
      const p1 = send(sToken, 'redeem', [10], {from});
      const p2 = send(sToken, 'mint', [10], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
  ...
  describe('#repayRepay', () => {
    it('should succeed', async () => {
      const sToken1 = await makeSToken({supportMarket: true, underlyingPrice: 1, collateralFactor: .5});
      const sToken2 = await makeSToken({supportMarket: true, underlyingPrice: 1, comptroller: sToken1.comptroller});
      await send(sToken1.underlying, 'harnessSetBalance', [from, 10]);
      await send(sToken1.underlying, 'approve', [sToken1._address, 10], {from});
      await send(sToken2.underlying, 'harnessSetBalance', [sToken2._address, 10]);
      await send(sToken2, 'harnessSetTotalSupply', [100]);
      await send(sToken2.underlying, 'approve', [sToken2._address, 10], {from});
      await send(sToken2, 'harnessSetExchangeRate', [etherMantissa(1)]);
      expect(await enterMarkets([sToken1, sToken2], from)).toSucceed();
      expect(await send(sToken1, 'mint', [10], {from})).toSucceed();
      expect(await send(sToken2, 'borrow', [2], {from})).toSucceed();
      await minerStop();
      const p1 = send(sToken2, 'repayBorrow', [1], {from});
      const p2 = send(sToken2, 'repayBorrow', [1], {from});
      await minerStart();
      expect(await p1).toSucceed();
      expect(await p2).toSucceed();
      expect((await borrowSnapshot(sToken2, from)).principal).toEqualNumber(0);
    });
