...
const { makeComptroller, makeSToken, makePriceOracle } = require('../Utils/Strike');
...
describe('ComptrollerV1', function() {
  ...
  describe('delegating to comptroller v1', () => {
    ...
    const maxAssets = 10;
    let unitrollerAsComptroller, sToken;

    beforeEach(async () => {
      unitrollerAsComptroller = await initializeBrains(oracle, etherMantissa(0.06), 30);
      sToken = await makeSToken({ comptroller: unitrollerAsComptroller });
    });
    ...
    describe('_setCollateralFactor', () => {
      ...
      it('fails if not called by admin', async () => {
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [sToken._address, half], {
            from: accounts[1]
          })
      ...
      it('fails if factor is too high', async () => {
        const sToken = await makeSToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [sToken._address, one])
      ...
      it('succeeds and sets market', async () => {
        const sToken = await makeSToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
        await send(oracle, 'setUnderlyingPrice', [sToken._address, 1]);
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [sToken._address, half])
    ...
    describe('_supportMarket', () => {
      it('fails if not called by admin', async () => {
        expect(
          await send(unitrollerAsComptroller, '_supportMarket', [sToken._address], { from: accounts[1] })
      ...
      it('succeeds and sets market', async () => {
        const result = await send(unitrollerAsComptroller, '_supportMarket', [sToken._address]);
        expect(result).toHaveLog('MarketListed', { sToken: sToken._address });
      });

      it('cannot list a market a second time', async () => {
        const result1 = await send(unitrollerAsComptroller, '_supportMarket', [sToken._address]);
        const result2 = await send(unitrollerAsComptroller, '_supportMarket', [sToken._address]);
      ...
      it('can list two different markets', async () => {
        const sToken1 = await makeSToken({ comptroller: unitroller });
        const sToken2 = await makeSToken({ comptroller: unitroller });
        const result1 = await send(unitrollerAsComptroller, '_supportMarket', [sToken1._address]);
        const result2 = await send(unitrollerAsComptroller, '_supportMarket', [sToken2._address]);
