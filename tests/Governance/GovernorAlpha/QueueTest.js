...
async function enfranchise(strk, actor, amount) {
  await send(strk, 'transfer', [actor, etherMantissa(amount)]);
  await send(strk, 'delegate', [actor], {from: actor});
}
...
describe('GovernorAlpha#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const strk = await deploy('STRK', [root]);
      const gov = await deploy('GovernorAlpha', [timelock._address, strk._address, root]);
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(strk, a1, 3e6);
      await mineBlock();

      const targets = [strk._address, strk._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(strk, a1, 3e6);
      await enfranchise(strk, a2, 3e6);
      await mineBlock();

      const targets = [strk._address];
      const values = ["0"];
