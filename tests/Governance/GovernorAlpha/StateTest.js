...
describe('GovernorAlpha#state/1', () => {
  let strk, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, strk._address, root]);
  ...
  it("Canceled", async () => {
    await send(strk, 'transfer', [accounts[0], etherMantissa(260753)]);
    await send(strk, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(strk, 'delegate', [root], { from: accounts[0] });
