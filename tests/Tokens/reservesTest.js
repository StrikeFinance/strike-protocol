...
const {fastForward, makeSToken} = require('../Utils/Strike');
...
describe('SToken', function () {
  ...
  describe('_setReserveFactorFresh', () => {
    ...
    it("rejects change by non-admin", async () => {
      expect(
        await send(sToken, 'harnessSetReserveFactorFresh', [factor], {from: accounts[0]})
    ...
    it("rejects change if market not fresh", async () => {
      ...
      expect(await send(sToken, 'harnessSetReserveFactorFresh', [factor])).toHaveTokenFailure('MARKET_NOT_FRESH', 'SET_RESERVE_FACTOR_FRESH_CHECK');
      expect(await call(sToken, 'reserveFactorMantissa')).toEqualNumber(0);
    });

    it("rejects newReserveFactor that descales to 1", async () => {
      expect(await send(sToken, 'harnessSetReserveFactorFresh', [etherMantissa(1.01)])).toHaveTokenFailure('BAD_INPUT', 'SET_RESERVE_FACTOR_BOUNDS_CHECK');
      expect(await call(sToken, 'reserveFactorMantissa')).toEqualNumber(0);
    });

    it("accepts newReserveFactor in valid range and emits log", async () => {
      const result = await send(sToken, 'harnessSetReserveFactorFresh', [factor])
    ...
    it("accepts a change back to zero", async () => {
      const result1 = await send(sToken, 'harnessSetReserveFactorFresh', [factor]);
  ...
  describe('_setReserveFactor', () => {
    ...
    it("emits a reserve factor failure if interest accrual fails", async () => {
      ...
      await expect(send(sToken, '_setReserveFactor', [factor])).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
      expect(await call(sToken, 'reserveFactorMantissa')).toEqualNumber(0);
    });

    it("returns error from setReserveFactorFresh without emitting any extra logs", async () => {
      const {reply, receipt} = await both(sToken, '_setReserveFactor', [etherMantissa(2)]);
  ...
  describe("_reduceReservesFresh", () => {
    ...
    beforeEach(async () => {
      sToken = await makeSToken();
      expect(await send(sToken, 'harnessSetTotalReserves', [reserves])).toSucceed();
      expect(
        await send(sToken.underlying, 'harnessSetBalance', [sToken._address, cash])
      ).toSucceed();
    });

    it("fails if called by non-admin", async () => {
      expect(
        await send(sToken, 'harnessReduceReservesFresh', [reduction], {from: accounts[0]})
    ...
    it("fails if market not fresh", async () => {
      expect(await send(sToken, 'harnessFastForward', [5])).toSucceed();
      expect(await send(sToken, 'harnessReduceReservesFresh', [reduction])).toHaveTokenFailure('MARKET_NOT_FRESH', 'REDUCE_RESERVES_FRESH_CHECK');
      expect(await call(sToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("fails if amount exceeds reserves", async () => {
      expect(await send(sToken, 'harnessReduceReservesFresh', [reserves.add(1)])).toHaveTokenFailure('BAD_INPUT', 'REDUCE_RESERVES_VALIDATION');
      expect(await call(sToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("fails if amount exceeds available cash", async () => {
      const cashLessThanReserves = reserves.sub(2);
      await send(sToken.underlying, 'harnessSetBalance', [sToken._address, cashLessThanReserves]);
      expect(await send(sToken, 'harnessReduceReservesFresh', [reserves])).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'REDUCE_RESERVES_CASH_NOT_AVAILABLE');
      expect(await call(sToken, 'totalReserves')).toEqualNumber(reserves);
    });

    it("increases admin balance and reduces reserves on success", async () => {
      const balance = etherUnsigned(await call(sToken.underlying, 'balanceOf', [root]));
      expect(await send(sToken, 'harnessReduceReservesFresh', [reserves])).toSucceed();
      expect(await call(sToken.underlying, 'balanceOf', [root])).toEqualNumber(balance.add(reserves));
      expect(await call(sToken, 'totalReserves')).toEqualNumber(0);
    });

    it("emits an event on success", async () => {
      const result = await send(sToken, 'harnessReduceReservesFresh', [reserves]);
  ...
  describe("_reduceReserves", () => {
    ...
    it("emits a reserve-reduction failure if interest accrual fails", async () => {
      await send(sToken.interestRateModel, 'setFailBorrowRate', [true]);
      await fastForward(sToken, 1);
      await expect(send(sToken, '_reduceReserves', [reduction])).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });

    it("returns error from _reduceReservesFresh without emitting any extra logs", async () => {
      const {reply, receipt} = await both(sToken, 'harnessReduceReservesFresh', [reserves.add(1)]);
      expect(reply).toHaveTokenError('BAD_INPUT');
