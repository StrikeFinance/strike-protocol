...
import "../../contracts/SErc20Immutable.sol";
import "../../contracts/SErc20Delegator.sol";
import "../../contracts/SErc20Delegate.sol";
import "../../contracts/SDaiDelegate.sol";
import "./ComptrollerScenario.sol";

contract SErc20Harness is SErc20Immutable {
    ...
    function harnessRedeemFresh(address payable account, uint sTokenAmount, uint underlyingAmount) public returns (uint) {
        return super.redeemFresh(account, sTokenAmount, underlyingAmount);
    }
    ...
    function harnessLiquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, SToken sTokenCollateral) public returns (uint) {
        (uint err,) = liquidateBorrowFresh(liquidator, borrower, repayAmount, sTokenCollateral);
        return err;
    }
...
contract SEvil is SErc20Scenario {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
    ...
    function evilSeize(SToken treasure, address liquidator, address borrower, uint seizeTokens) public returns (uint) {
        return treasure.seize(liquidator, borrower, seizeTokens);
    }
}

contract SErc20DelegatorScenario is SErc20Delegator {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract SErc20DelegateHarness is SErc20Delegate {
...
contract SErc20DelegateScenarioExtra is SErc20DelegateScenario {
...
contract SDaiDelegateHarness is SDaiDelegate {
    uint blockNumber = 100000;
    uint harnessExchangeRate;
    bool harnessExchangeRateStored;
...
contract SDaiDelegateScenario is SDaiDelegate {
    function setTotalBorrows(uint totalBorrows_) public {
        totalBorrows = totalBorrows_;
    }
...
contract SDaiDelegateMakerHarness is PotLike, VatLike, GemLike, DaiJoinLike {
    /* Pot */
