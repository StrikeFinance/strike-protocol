...
contract ComptrollerG1 is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function enterMarkets(address[] memory sTokens) public returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            accountAssets[msg.sender].push(sToken);

            emit MarketEntered(sToken, msg.sender);
    ...
    function redeemAllowed(address sToken, address redeemer, uint redeemTokens) external returns (uint) {
        return redeemAllowedInternal(sToken, redeemer, redeemTokens);
    }
    ...
    function liquidateBorrowVerify(
        ...
        sTokenBorrowed;   // currently unused
        sTokenCollateral; // currently unused
        liquidator;       // currently unused
        borrower;         // currently unused
        repayAmount;      // currently unused
    ...
    function transferAllowed(address sToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        return redeemAllowedInternal(sToken, src, transferTokens);
    }
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            // sumCollateral += tokensToEther * sTokenBalance
            (mErr, vars.sumCollateral) = mulScalarTruncateAddUInt(vars.tokensToEther, vars.sTokenBalance, vars.sumCollateral);
            if (mErr != MathError.NO_ERROR) {
                return (Error.MATH_ERROR, 0, 0);
            }
     ...
     * @param repayAmount The amount of sTokenBorrowed underlying to convert into sTokenCollateral tokens
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure. (See ErrorReporter for details)
      */
    function _setCollateralFactor(SToken sToken, uint newCollateralFactorMantissa) external returns (uint256) {
    ...
    function _supportMarket(SToken sToken) external returns (uint) {
        ...
        markets[address(sToken)] = Market({isListed: true, collateralFactorMantissa: 0});
