...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function _become(Unitroller unitroller, uint strikeRate_, address[] memory strikeMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == unitroller.admin(), "only unitroller admin can change brains");
        require(unitroller._acceptImplementation() == 0, "change not authorized");

        ComptrollerG3(address(unitroller))._becomeG3(strikeRate_, strikeMarketsToAdd, otherMarketsToAdd);
    }

    function _becomeG3(uint strikeRate_, address[] memory strikeMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == comptrollerImplementation, "only brains can become itself");

        for (uint i = 0; i < strikeMarketsToAdd.length; i++) {
            _addMarketInternal(address(strikeMarketsToAdd[i]));
        }
        ...
        _setStrikeRate(strikeRate_);
        _addStrikeMarkets(strikeMarketsToAdd);
    }
    ...
    function refreshStrikeSpeeds() public {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            if (markets[address(sToken)].isStriked) {
                ...
                Exp memory interestPerBlock = mul_(Exp({mantissa: sToken.borrowRatePerBlock()}), sToken.totalBorrows());
