...
import { sTokenFetchers, getSTokenValue } from './Value/STokenValue';
import { sTokenDelegateFetchers, getSTokenDelegateValue } from './Value/STokenDelegateValue';
...
import { getStrkValue, strkFetchers } from './Value/STRKValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isStrikeKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isStrikeStr = await world.web3.eth.getStorageAt(addr.val, isStrikeKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isStrikeStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (SToken sZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (SToken sZRX Comptroller) (Comptroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "SToken ...sTokenArgs" - Returns sToken value
    `,
    'SToken',
    [new Arg('res', getSTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: sTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### STokenDelegate

      * "STokenDelegate ...sTokenDelegateArgs" - Returns sToken delegate value
    `,
    'STokenDelegate',
    [new Arg('res', getSTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: sTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Erc20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "STRK ...compArgs" - Returns STRK value
    `,
    'STRK',
    [new Arg('res', getStrkValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: strkFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "StrikeLens", implicit: false },
