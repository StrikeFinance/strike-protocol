...
export function printHelp(printer: Printer, event: Event, expressions: Expression<any>[], path: string[]=[]) {
  if (event.length === 0) {
    ...
    if (path.length === 0) {
      banner = (
`
## Strike Command Runner

The Strike Command Runner makes it easy to interact with Strike. You can input simple commands
...
sub-commands run \`Help SToken\` or \`Help SToken Mint\`.
`).trim();
    } else {
