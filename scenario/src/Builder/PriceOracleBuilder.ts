...
const StrikePriceOracle = getContract('StrikePriceOracle');
...
export async function buildPriceOracle(world: World, from: string, event: Event): Promise<{world: World, priceOracle: PriceOracle, priceOracleData: PriceOracleData}> {
  const fetchers = [
    ...
    new Fetcher<{}, PriceOracleData>(`
        ...
        * "Strike" - The a strike price oracle that using band protocol
      ...
      async (world, {}) => {
        return {
          invokation: await StrikePriceOracle.deploy<PriceOracle>(world, from, []),
          description: "Strike Price Oracle"
        };
      }
    ),
