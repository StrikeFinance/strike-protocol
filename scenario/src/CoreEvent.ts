...
import { sTokenCommands, processSTokenEvent } from './Event/STokenEvent';
import { sTokenDelegateCommands, processSTokenDelegateEvent } from './Event/STokenDelegateEvent';
...
import { strkCommands, processSTRKEvent } from './Event/STRKEvent';
...
import { StrikeLens } from './Contract/StrikeLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ blockNumber: NumberV, event: EventV }>(
    `
      ...
      * "Block 10 (...event)" - Set block to block N and run event
        * E.g. "Block 10 (STRK Deploy Admin)"
    `,
    'Block',
    [
      new Arg('blockNumber', getNumberV),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (SToken sEth Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (SToken sZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ given: Value; event: EventV }>(
    `
      ...
      * "Given value event" - Runs event only if value is truthy. Thus, given can be used to build existence checks.
        * E.g. "Given ($var) (PriceOracle SetPrice sBAT $var)"
    `,
    'Given',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "SToken ...event" - Runs given SToken event
        * E.g. "SToken sZRX Mint 5e18"
    `,
    'SToken',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processSTokenEvent(world, event.val, from),
    { subExpressions: sTokenCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "STokenDelegate ...event" - Runs given STokenDelegate event
        * E.g. "STokenDelegate Deploy SDaiDelegate sDaiDelegate"
    `,
    'STokenDelegate',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processSTokenDelegateEvent(world, event.val, from),
    { subExpressions: sTokenDelegateCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (SToken sETH Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (SToken sETH Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "STRK ...event" - Runs given strk event
      * E.g. "STRK Deploy"
    `,
    'STRK',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processSTRKEvent(world, event.val, from);
    },
    { subExpressions: strkCommands() }
  ),
  ...
  buildContractEvent<StrikeLens>("StrikeLens", false),
