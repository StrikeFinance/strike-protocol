...
async function getRefAddress(world: World, priceOracle: PriceOracle): Promise<AddressV> {
  return new AddressV(await priceOracle.methods.getRefAddress().call());
}
...
export function priceOracleFetchers() {
  return [
    ...
    new Fetcher<{priceOracle: PriceOracle}, AddressV>(`
        ...
        * "RefAddress" - Gets the ref address of the price oracle
      `,
      "RefAddress",
      [
        new Arg("priceOracle", getPriceOracle, {implicit: true})
      ],
      (world, {priceOracle}) => getRefAddress(world, priceOracle)
    )
  ];
}
