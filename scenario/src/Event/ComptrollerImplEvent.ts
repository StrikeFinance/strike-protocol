...
async function becomeG3(
  ...
  comptrollerImpl: ComptrollerImpl,
  unitroller: Unitroller,
  strikeRate: encodedNumber,
  strikeMarkets: string[],
  otherMarkets: string[]
): Promise<World> {
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, strikeRate, strikeMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      ...
      comptrollerImpl: ComptrollerImpl;
      strikeRate: NumberV;
      strikeMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <StrikeMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl BecomeG3 0.1e18 [sDAI, sETH, sUSDC]
      `,
      'BecomeG3',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('strikeRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('strikeMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <StrikeMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl Become 0.1e18 [sDAI, sETH, sUSDC]
      `,
      'Become',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
