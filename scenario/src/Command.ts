        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (SToken Eth Address) (... deploy sToken)`
              // could be used to deploy a sToken if it doesn't exist, but
              // since there is no SToken, that check would raise (when we'd
